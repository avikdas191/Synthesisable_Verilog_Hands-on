...
//	}
	...
	//return MAIN_RETURN_VAL;
	extern float CoreMark; //#
	CoreMark = default_num_contexts*(double)results[0].iterations/(double)time_in_secs(total_time);
	//printf("* time= %f \n num = %d\n iter = %d\n", time_in_secs(total_time), default_num_contexts, results[0].iterations);
	//printf("* CoreMark = %f\n", (float)default_num_contexts*(double)results[0].iterations/(double)time_in_secs(total_time));
