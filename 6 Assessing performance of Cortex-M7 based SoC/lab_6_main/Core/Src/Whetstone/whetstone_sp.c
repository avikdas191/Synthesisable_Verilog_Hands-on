...
int whetstone_benchmark(int loop_in, int iter_in) // #added parameters
{
...
C
	LOOP = 1000;
*/
	LOOP = loop_in;	// #added parameters
	II   = iter_in;	// #added parameters
...
*/
	...
	for (I = 1; I <= N7; I++) {
		X = T * DATAN(t2*DSIN(X)*DCOS(X)/(DCOS(X+Y)+DCOS(X-Y)-1.0));  // # changed case from T2 to t2
		Y = T * DATAN(t2*DSIN(Y)*DCOS(Y)/(DCOS(X+Y)+DCOS(X-Y)-1.0));	// # changed case from T2 to t2
	}
